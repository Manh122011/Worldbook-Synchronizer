  ...
  $('#wb-sync-sync-all-btn').on('click', async function () {
    ...
    setTimeout(
      () =>
        $(this)
          .prop('disabled', false)
          .html('<i class="fa-solid fa-cloud-arrow-up"></i> Đồng bộ hóa mọi thứ chỉ bằng một cú nhấp chuột'),
      2000,
    );
  });
...
export async function populateSyncWorldbooks() {
  try {
    ...
    $targetWbSelect.empty().append('<option value="">-- Vui lòng chọn sách thế giới mục tiêu --</option>');
    books.forEach(book => $targetWbSelect.append(`<option value="${escapeHtml(book.file_name)}">${escapeHtml(book.name)}</option>`));
  } catch (error) {
    $targetWbSelect.empty().append('<option value="">Tải không thành công</option>');
  }
}
...
export async function extractContentFromMessage(startTag, endTag, floorInput) {
  if (!startTag || !endTag) {
    toastr.warning('Vui lòng điền thẻ bắt đầu và kết thúc');
    return null;
  }
  try {
    ...
    if (messages.length === 0) {
      toastr.warning('Thông báo không tìm thấy tầng được chỉ định');
      return null;
    }
    ...
    if (allExtractedTexts.length === 0) {
      toastr.info(`Không tìm thấy ${startTag} Và ${endTag} Nội dung gói`);
      return null;
    }
    return allExtractedTexts;
  } catch (error) {
    toastr.error('Không thể giải nén: ' + error.message);
    return null;
  }
}
...
export async function extractMessages() {
  ...
  renderCards();
  toastr.success(`trích xuất thành công ${extractedCards.length} mặt hàng`);
}

export function parseExtractedText(text, index) {
  let name = `Trích xuất các mục ${index + 1}`,
    keys = '',
    content = text;
  ...
  if (name !== `Trích xuất các mục ${index + 1}` || keys !== '') content = contentLines.join('\n').trim();
...
export function renderCards() {
  if (extractedCards.length === 0) {
      $cardsContainer.html('<div class="wb-sync-empty-msg">Không có mục nào được truy xuất.</div>');
      return;
  }
      ...
      cardsHtml += `
          <div class="wb-sync-card" data-id="${card.id}" style="border: 1px solid var(--wb-sync-border); border-radius: 5px; background: rgba(0,0,0,0.2); overflow: hidden;">
              ...
              <div class="wb-sync-card-content" style="${contentStyle}">
                  ...
                  <div class="wb-sync-card-row" style="margin-top: 10px;"><span class="wb-sync-card-label">Từ khóa</span><input type="text" class="wb-sync-card-input card-keys" value="${escapeHtml(card.keys)}" placeholder="được phân tách bằng dấu phẩy"></div>
                  ...
                  <div class="wb-sync-card-row">
                      ...
                      <select class="wb-sync-card-select card-mode">
                          <option value="constant" ${card.mode === 'constant' ? 'selected' : ''}>Vĩnh viễn (ánh sáng xanh)</option>
                          <option value="selective" ${card.mode === 'selective' ? 'selected' : ''}>Trình kích hoạt có điều kiện (đèn xanh)</option>
                      </select>
                  </div>
                  <div class="wb-sync-card-row">
                      <span class="wb-sync-card-label">vị trí chèn</span>
                      <select class="wb-sync-card-select card-position">
                          <option value="0" ${card.position == 0 ? 'selected' : ''}>Trước khi xác định vai trò</option>
                          <option value="1" ${card.position == 1 ? 'selected' : ''}>Sau khi xác định vai trò</option>
                          <option value="2" ${card.position == 2 ? 'selected' : ''}>trước tin nhắn mẫu (↑EM)</option>
                          <option value="3" ${card.position == 3 ? 'selected' : ''}>Sau tin nhắn mẫu (↓EM)</option>
                          <option value="4" ${card.position == 4 ? 'selected' : ''}>Trước khi tác giả giải thích</option>
                          <option value="5" ${card.position == 5 ? 'selected' : ''}>Sau khi tác giả giải thích</option>
                          <option value="6" ${card.position == 6 ? 'selected' : ''}>@D◆[hệ thống]sâu sắc</option>
                          <option value="7" ${card.position == 7 ? 'selected' : ''}>@D[người dùng]sâu sắc</option>
                          <option value="8" ${card.position == 8 ? 'selected' : ''}>@D[AI]sâu sắc</option>
                      </select>
                  </div>
                  ...
                  <div class="wb-sync-card-row">
                      <span class="wb-sync-card-label">Cài đặt đệ quy</span>
                      <div style="display: flex; gap: 10px;">
                          <label class="wb-sync-checkbox-label"><input type="checkbox" class="card-prevent-in" ${card.preventIncoming ? 'checked' : ''}> không thể tái diễn</label>
                          <label class="wb-sync-checkbox-label"><input type="checkbox" class="card-prevent-out" ${card.preventOutgoing ? 'checked' : ''}> ngăn chặn sự tái diễn thêm</label>
                      </div>
                  </div>
                  <div class="wb-sync-card-actions">
                      <button class="wb-sync-button wb-sync-btn-small sync-single-btn">Đồng bộ hóa mục này</button>
                      <button class="wb-sync-button wb-sync-btn-small abandon delete-card-btn">xóa bỏ</button>
                  </div>
              </div>
          </div>
      `;
...
export function buildEntryData(cardData) {
  ...
  let keysArray =
    !isConstant && cardData.keys
      ...
      : [];
  return {
    name: cardData.name || 'Mục nhập không tên',
...
export async function syncEntries(cardsToSync, $btn = null) {
  ...
  if (!targetWb) return toastr.warning('Vui lòng chọn sách thế giới mục tiêu trước');
  if (cardsToSync.length === 0) return toastr.warning('Không có mục nào để đồng bộ hóa');
  if ($btn) $btn.prop('disabled', true).text('Đồng bộ hóa...');
  try {
    const api = await getTavernHelper();
    const newEntries = cardsToSync.map(c => buildEntryData(c));
    await api.createLorebookEntries(targetWb, newEntries);
    toastr.success(`Đã đồng bộ hóa thành công ${newEntries.length} mặt hàng`);
    if ($btn) $btn.text('Đã đồng bộ hóa').css({ 'border-color': 'var(--wb-sync-primary)', color: 'var(--wb-sync-primary)' });
  } catch (e) {
    toastr.error(`Đồng bộ hóa không thành công: ${e.message}`);
    if ($btn)
      $btn
        .prop('disabled', false)
        .text('Đồng bộ hóa không thành công')
