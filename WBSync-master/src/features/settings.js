...
export function saveSettings(event) {
  ...
  if (!settings.showFloatingBtn && !settings.showMagicWandBtn && !settings.showQrBtn) {
    toastr.warning('Cần phải đặt trước ít nhất một mục bổ trợ！');
...
export function applySettings(settings) {
  ...
  if (settings.showMagicWandBtn !== false) {
      if ($(`#${menuId}`).length === 0) {
          const menuItemHtml = `
              <div id="${menuId}" class="list-group-item flex-container flexGap5" title="Đồng bộ hóa sách thế giới mở">
                  <i class="fa-solid fa-book-atlas fa-fw"></i>
                  <span>đồng bộ hóa sách thế giới</span>
              </div>
          `;
...
export function initQrMenu() {
  ...
  const qrInterval = setInterval(() => {
      ...
      if ($qrBar.length > 0) {
          const qrItemHtml = `
              <div id="${qrMenuId}" class="menu_button qr--button" title="Đồng bộ hóa sách thế giới mở" style="cursor: pointer; display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; margin-right: 5px; background: rgba(255,255,255,0.1); border-radius: 5px;">
          ...
          if (qrRetryCount > 20) {
              ...
              if ($sendTextarea.length > 0) {
                   const qrItemHtml = `
                      <div id="${qrMenuId}" class="menu_button" title="Đồng bộ hóa sách thế giới mở" style="cursor: pointer; display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; margin-bottom: 5px; background: rgba(255,255,255,0.1); border-radius: 5px;">
                          <i class="fa-solid fa-book-atlas"></i>
                      </div>
                  `;
